...
const links: ILinks[] = [
  { href: "#home", label: <FaHouse /> },
  { href: "#things-i-do", label: <FaLaptopCode /> },
  { href: "#experience", label: <FaBusinessTime /> },
  { href: "#works", label: <GoProjectRoadmap /> },
  { href: "#contact", label: <IoMdCall /> },
]
...
export default function Navbar() {
  return (
    <nav className="flex items-center justify-between md:justify-center gap-6 px-6 py-4 md:py-6 fixed w-full md:w-fit z-40 top-0 right-1/2 translate-x-1/2 bg-[rgb(0,0,0,0.9)] border border-[rgb(200,200,200,0.2)] bg-blur-120 md:rounded-b-[10px]">
      {links.map((link, i) => (
        <a key={i} href={link.href} className="text-3xl hover:text-white text-[rgb(120,120,120)]">{link.label}</a>
      ))}
    </nav>
  )
}